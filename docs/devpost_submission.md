...
- **Live parameter lookup via SerpApi**: for the steel-beam scenario it searches the web for typical values and prefills them.
...
- The whole product is testable headlessly: 87 unit tests + an AppTest smoke suite that drives the actual Streamlit UI (manual mode, natural-language mode with mocked LLM, history, export) in both languages.
